...
![Source: http://www.saravanabhavan.com/restaurants.php](/img/hsb_original.png)
...
library(DT)
library(widgetframe)
hsb1 <- readRDS("../../static/data/hsb1.rds")
hsb1_dt <- datatable(
    hsb1, options = list(
...
frameWidget(hsb1_dt, height = 350, width = '95%')
```
...
```{r echo=FALSE}
hsb2 <- readRDS("../../static/data/hsb2.rds")
hsb2_dt <- datatable(
   hsb2 %>%
...
frameWidget(hsb2_dt, height = 350, width = '95%')
```
...
frameWidget(hsb_leaflet, height = '400')
```
...
frameWidget(date_dt, height = 350, width = '95%')
```
